/**
Стадию Пресейл - убрать.
Распределение токенов изменить
Периоды - изменить
Курс - изменить
Расчет бонусов - изменитиь
Новая функция - Все токены отправленные после окончания токен Sale на смартконтракт , должны сжигаться. - 
- задача реализована по другому (согласовано) - функция acceptTokensFromUsers забирает токены у пользователя и сжигает их.
*/
...
contract Crowdsale is Ownable {
    ...
    */
    ...
    function setBonus () public view returns(uint256) {
        ...
        if ((uint64(now) >= crowdSaleStartTime) && (uint64(now) < crowdSaleStartTime + 31 days)){
            actualBonus = 35;
        }
        if ((uint64(now) >= crowdSaleStartTime + 31 days) && (uint64(now) < crowdSaleStartTime + 61 days)){
            actualBonus = 15;
        }
